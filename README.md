...
- 스테이지를 클리어하면 플레이 경로가 저장됩니다.
- 다음 플레이부터 같은 스테이지에서 `내 최고` 고스트가 반투명하게 같이 달립니다.
...
2. GitHub 저장소 Secrets에 `SUPABASE_URL`, `SUPABASE_ANON_KEY`를 추가합니다.
3. 선택으로 Variables에 `SUPABASE_TABLE`, `BOUNCE_PLAYER_NAME`을 추가합니다.
4. 다시 배포하면 온라인 1등 기록 업로드와 고스트 다운로드가 켜집니다.
